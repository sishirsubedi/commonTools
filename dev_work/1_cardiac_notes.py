...
## - 1_cardiac_genes_unq_withHMGD_RefSeq_manual.csv -- get information from vcf file manually
...
for indx,row in df.iterrows():
    ...
    for indx2 in range(len(cdstart)):
        temp.append([row['Gene'],row['HMGD_RefSeq'],row['chrom'],
        row['strand'],row['exonCount'],row['txStart'],row['txEnd'],
        row['cdsStart'],row['cdsEnd'],int(cdstart[indx2]),int(cdend[indx2]),
        row['exonStarts'],row['exonEnds']])
df_filt = pd.DataFrame(temp)
df_filt.columns = ['Gene','HMGD_RefSeq','chr','strand','exonCount','txStart','txEnd','cdsStart','cdsEnd','coding_start','coding_end','exonStarts','exonEnds']
df_filt = df_filt[['chr','coding_start','coding_end','Gene','HMGD_RefSeq','strand','exonCount','txStart','txEnd','cdsStart','cdsEnd','exonStarts','exonEnds']]
...
file_in = 'exome_samples/Exome18-N.cardiac_intersect.bed'
file_out = 'exome_samples/Exome18-N.cardiac_result.txt'
